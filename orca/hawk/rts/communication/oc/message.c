...
new_message(int header_size, int max_data_size)
{
  ...
  message=(message_p)pan_malloc(sizeof(message_t));
  assert(message!=NULL);
  assert((header_size % 8)==0);
  assert((sizeof(message_header_t) % 8)==0);
  message->message=(void *)pan_malloc(header_size+max_data_size+
...
expand_message(message_p message, int max_data_size) {
  ...
  assert(initialized);
  assert(message!=NULL);
  assert(message->message_header!=NULL);
  assert(max_data_size>=0);

  if (message->max_user_data_size>=max_data_size) return 0;
  temp=pan_malloc(message->message_header->user_header_size+
	      max_data_size+sizeof(message_header_t));
  assert(temp!=NULL);
  memcpy(temp,message->message,
	 sizeof(message_header_t)+
	 message->message_header->user_header_size+
