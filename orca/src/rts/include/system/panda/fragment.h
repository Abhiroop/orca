...
#define f_get_oid(f, oid)            oid_copy(&((f)->fr_oid), oid)
...
/* Lock operations.
 */
#define f_lock(f)        sys_mutex_lock(f_get_lock(f))
#define f_try_lock(f)    sys_mutex_trylock(f_get_lock(f))
#define f_unlock(f)      sys_mutex_unlock(f_get_lock(f))
...
extern int f_read(fragment_p f, struct type_descr *optype, int opindex,
		  void **args, source_t src);

extern int f_read_write(fragment_p f, struct type_descr *optype,
			int opindex, void **args, int *modified, source_t src);

extern int f_write_read(fragment_p f, struct type_descr *optype, int opindex,
