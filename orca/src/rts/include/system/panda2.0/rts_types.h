...
typedef int (*cont_func_p)(void *state, pan_mutex_p lock);
...
typedef void (*oper_p)(int op, pan_upcall_p upcall,
			    pan_msg_p request);
...
struct manager {
    struct {
        float write_sum;     /* sum of write scores */
        float access_sum;    /* sum of access scores */
        float max_access;    /* maximum access value over all platforms */
...
struct piggy_info {
    float access_sum;        /* sum of compile-time access scores */
    float nr_accesses;       /* #reads + #writes up until now */
    float delta_reads;       /* #local reads since last external operation */
};
...
struct continuation {
    ...
    int         (*c_func)(void *state, pan_mutex_p lock);  
...
struct cont_queue {
    pan_mutex_p          cq_lock;            /* queue lock                 */
...
struct invocation {
    ...
    pan_upcall_p   upcall;    /* RPC upcall id */
...
struct process {
    ...
    pan_mutex_p         p_lock;     /* protects p_nwrites */
