/* $Id: unix.c,v 1.11 1995/08/23 10:39:32 ceriel Exp $ */
...
f_unix__read(int fildes, t_array *buffer, int nbytes)
{
  if (buffer->a_sz != nbytes) {
	if (buffer->a_sz > 0) a_free(buffer, &td_string);
	else buffer->a_offset = 1;
	a_allocate(buffer, &td_string, buffer->a_offset, buffer->a_offset+nbytes-1);
  }
...
f_unix__ReadString(int fildes, t_array *res)
{
    ...
    a_free(res, &td_string);
    a_allocate(res, &td_string, 1, n);
