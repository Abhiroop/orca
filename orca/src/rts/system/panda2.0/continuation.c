...
pan_mutex_p cont_immediate_lock;
...
cont_end(void)
{
	...
	pan_cond_clear(cont_immediate_work);
    }
    
    cont_clear(&cont_immediate_queue);
    pan_mutex_clear(cont_immediate_lock);
}
 ...
 */
void
cont_init(cont_queue_t *q, pan_mutex_p lock)
{
    q->cq_lock      = lock;
    q->cq_freelist  = 0;
    q->cq_contlist  = 0;
...
cont_clear(cont_queue_t *q)
{
	...
	pan_cond_clear(cont->c_cond);
	m_free(cont);
    }

    pan_mutex_unlock(q->cq_lock);
}
 ...
 */
void *
cont_alloc(cont_queue_t *q, int size,
	   int (*func)(void *state, pan_mutex_p lock))
{
    unsigned i;
    cont_t *cont;

    assert(size <= CONT_SIZE);
    assert(!pan_mutex_trylock(q->cq_lock));     /* lock must be held! */

    if (!q->cq_freelist) {
	for (i = 0; i < EXTEND_FREE_LIST; i++) {
	    cont = (cont_t *)m_malloc(sizeof(*cont));
	    (void)memset(cont, 0, sizeof(*cont));
	    cont->c_cond = pan_cond_create(q->cq_lock);
...
cont_save(void *buf, int block_me)
{
    ...
    assert(!pan_mutex_trylock(q->cq_lock));
