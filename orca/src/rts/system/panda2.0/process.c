...
{
    process_p proc = (process_p)m_malloc(sizeof(process_t));
	...
	op_threads = (process_p *)m_realloc(op_threads,
    ...
    proc->p_pipelined = 0;
    proc->p_nwrites = 0;
    proc->p_lock = pan_mutex_create();
    proc->p_flushed = pan_cond_create(proc->p_lock);
#endif
...
p_clear(process_p proc)
{
    ...
    pan_cond_clear(proc->p_flushed);
    pan_mutex_clear(proc->p_lock);
#endif
}
...
p_start(void)
{
    ...
    maxthreads = DELTA_THREADS;
    op_threads = (process_p *)m_malloc(maxthreads * sizeof(process_p));
}
