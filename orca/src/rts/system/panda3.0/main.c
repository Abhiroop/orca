...
static void
comm_handler(int handle, int upcall, void *request, int size, int len)
{
    int mc_done = 0;
	...
	(void)rc_msg_push(&request, &size, &len, BIG);
...
sync_platforms(int *pargc, char **argv)
{
    ...
    (void)rc_msg_push(&request, &req_size, &req_len, BIG);
	...
	for (;;) {
	    ...
	    rc_rpc(cpu, rpc_handle, request, req_len, &reply, &size, &len);
	    pan_free(reply);
	    cpu++;
	}
    }
	 ...
	 */
	rc_mcast(mcast_handle, request, req_len);
    } else {
	pan_free(request);
    }
