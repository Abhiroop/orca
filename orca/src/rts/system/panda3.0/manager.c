...
static void r_replicate(int op, int upcall, void *request, int size, int len);
static void r_migrate(int op, int upcall, void *request, int size, int len);
static void r_fetch_obj(int op, int upcall, void *request, int size, int len);
static void r_delete_ref(int op, int upcall, void *request, int size, int len);
...
man_mcast_cont(void *state)
{
     ...
     */
    if (man_alive) {
	rc_mcast(mc->handle, mc->msg, mc->len);
    }
...
static void
man_mcast(int handle, void *msg, int size, int len)
{
...
man_delete( fragment_p objects, float reads, float writes, int nr)
{
    ...
    int do_delete = 0;
    int *n_delete;
    void *msg = rc_msg_create(&size, &len);
    delete_hdr_t *hdr;
	...
	if ( f_get_status(f) != f_unshared) {
		...
		hdr = rc_msg_push(&msg, &size, &len, sizeof(delete_hdr_t));
		hdr->writes = writes;
		hdr->reads  = reads;
	 ...
	 * the exit message!
	 */
	n_delete = rc_msg_push(&msg, &size, &len, sizeof(int));
	*n_delete = do_delete;
	rc_mcast(delete_handle, msg, len);
    }
}
 ...
 */
{
	...
	if ( rpc < ((float)0.8)*bcast && !replicate_all) {
		...
		hdr = rc_msg_push(&msg, &size, &len, sizeof(migrate_hdr_t));
		hdr->sender = rts_my_pid;
		hdr->max_location = location;
		hdr->oid = obj->fr_oid;
		man_mcast(exclusive_handle, msg, size, len);
	...
	if (!replicate_none && (bcast < ((float)0.9)*rpc || replicate_all)) {
		...
		mm_pack_sh_object(&msg, &size, &len, obj);
		hdr = rc_msg_push(&msg, &size, &len, sizeof(replicate_hdr_t));
		hdr->sender = rts_my_pid;
		hdr->oid    = obj->fr_oid;
		man_mcast(replicate_handle, msg, size, len);
		...
		man_mcast(migrate_handle, msg, size, len);
...
static void
r_replicate(int handle, int upcall, void *request, int size, int len)
{
    replicate_hdr_t *hdr;
    fragment_p obj;
    ...
    hdr = rc_msg_pop(request, &len, sizeof(replicate_hdr_t));
	...
	if (hdr->sender == rts_my_pid) {
	    ...
	    mm_unpack_sh_object(request, &len, obj);
...
r_migr_cont(void *state)
{
    ...
    req = rc_msg_create(&req_size, &req_len);
    tm_push_int(&req, &req_size, &req_len, obj->fr_oid);
    tm_push_int(&req, &req_size, &req_len, rts_my_pid);
     ...
     * fetch object data
     */
    rc_rpc(src, fetch_handle, req, req_len, &rep, &rep_size, &rep_len); 

    mm_unpack_sh_object(rep, &rep_len, obj);
...
static void
r_migrate(int handle, int dummy, void *request, int size, int len)
{
    fragment_p obj;
    ...
    hdr = rc_msg_pop(request, &len, sizeof(migrate_hdr_t));
...
static void
r_fetch_obj(int handle, int upcall, void *request, int size, int len)
{
    int oid;
    fragment_p obj;
    int cpu, mc_done;
     ...
     */
    obj->fr_owner = cpu;
    mm_pack_sh_object(&reply, &size, &len, obj);
    assert(obj->fr_manager == NULL);
    rts_man_check(obj);

    rc_untagged_reply(upcall, reply, len);
    pan_free(request);
...
static void
r_delete_ref(int handle, int dummy, void *request, int size, int len)
/*
 ...
 */
{
    ...
    n_delete = rc_msg_pop(request, &len, sizeof(int));
    do_delete = *n_delete;
    for ( i=0; i<do_delete; i++) {
	hdr = rc_msg_pop(request, &len, sizeof(delete_hdr_t));
