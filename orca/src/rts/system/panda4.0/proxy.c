...
static void clear_reply_msg(rc_msg_p m)
{
    rts_lock();
    rc_proto_clear(m->proto);
    mm_free_iovec(m->iov);
    mm_free_op_args(m->op, m->argv);
    rts_unlock();
    rc_msg_clear(m);
}

INLINE void
send_result(int upcall, int success, op_dscr *op, void **argv)
{
    struct reply_cont *rc;
    int len = 0;
    rc_msg_p	reply = rc_msg_create();

/*    rts_measure_enter(marshall_reply_timer); */
    reply->proto = rc_proto_create();
    reply->proto_size = mm_res_proto_top;
    reply->clearfunc = clear_reply_msg;
    reply->argv = argv;
    reply->op = op;

    reply->iov = mm_pack_op_result(&reply->iov_size, reply->proto, success, op, argv);
...
cont_rpc_invocation(void *state)
{
	...
	success = 1;
#endif
	send_result(upcall, success, op, argv); 
	i_clear(i);
	trc_event(cont_done, &continf);
	return (modified ? CONT_RESTART : CONT_NEXT);
    }
...
static int 
r_rpc_invocation(int _op, int upcall, pan_msg_p request, void *proto)
{
...
/*    rts_measure_enter(unmarshall_req_timer); */
    assert(_op == rpc_handle);

#ifdef PURE_WRITE
    hdr = mm_unpack_op_call(request, proto, &f, &op, &argv, &piggy_info,
			    &pure_write);
#else
    hdr = mm_unpack_op_call(request, proto, &f, &op, &argv, &piggy_info);
#endif
 ...
 */
static int 
r_mcast_invocation(int _op, int dummy, pan_msg_p request, void *proto)
{
    cont_queue_t *cont_queue;
    fragment_p f;
    f_status_t status;
		      ...
		      int opindex, void **argv, int *op_flags)
{
	...
	*p = &sync_inv;
	...
	iov = mm_pack_op_call(&iov_size, proto, f, op, argv, &sync_inv, 1);

	start_next_write(p_self(), f);                /* try to pipeline */
	trc_event(pure_write, &opinf);

	rc_mcast(mcast_handle, iov, iov_size, proto, mm_comb_proto_top);
	return 1;                                 /* trouble? */
    }
#endif
...
/*     rts_measure_enter(marshall_req_timer); */
    ...
    proto = rc_proto_create();
    iov = mm_pack_op_call(&req_size, proto, f, op, argv, 0, 0);
...
/*     rts_measure_leave(marshall_req_timer); */
    ...
    rc_rpc(owner, rpc_handle, iov, req_size, proto, mm_comb_proto_top,
    	   &reply, &rep_proto);
/*     rts_measure_enter(unmarshall_reply_timer); */
    rts_unlock();
    mm_unpack_op_result(reply, rep_proto, op, &success, argv);
    pan_msg_clear(reply);
    rts_lock();
