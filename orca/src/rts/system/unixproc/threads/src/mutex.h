...
#define mac_change_unlock_prio_wake(mutex, p, locked) \
  MACRO_BEGIN \
    if (mutex->protocol == PRIO_PROTECT) { \
      ...
      p->attr.prio = MAX(p->max_ceiling_prio, p->base_prio); \
      if (locked) { \
        if (mutex->queue.head->new_prio != MUTEX_WAIT) \
          mutex->queue.head->base_prio = mutex->queue.head->new_prio; \
        mutex->queue.head->new_prio = NO_PRIO; \
        mac_change_lock_prio(mutex, mutex->queue.head); \
        pthread_q_wakeup(&mutex->queue, PRIMARY_QUEUE); \
      } \
      if (ready.head != ready.tail) \
        pthread_mutex_q_adjust(); \
    } \
    else \
      if (locked) \
        pthread_q_wakeup(&mutex->queue, PRIMARY_QUEUE); \
