...
typedef struct pan_fragment *pan_fragment_p; 
...
also provides the primitives to fragment a message in pieces that can be
sent by the underlying architecture in one packet. All fragments of a message
will reserve some space to put a header in front of it. The size of the
header is specified in the network service access point associated with
the message when it is fragmented. This nsap will also become associated
with the fragment.
...
extern pan_fragment_p pan_msg_fragment(pan_msg_p msg, pan_nsap_p nsap);
/*
   Fragment message {\em msg}. Network service access point {\em nsap\/}
   becomes associated with the fragment that is returned.
*/

extern pan_fragment_p pan_msg_next(pan_msg_p msg);
/*
   Puts the next data part of message {\em msg} in the fragment
   associated with {\em message\/} in {\em pan\_msg\_fragment\/}. Returns
   a pointer to the fragment associated with this message if {\em msg\/}
   was not completely fragmented yet. Otherwise, it returns NULL.
*/

extern void pan_msg_assemble(pan_msg_p msg, pan_fragment_p frag,
			     int preserve);
/*
   Appends {\em frag} to {\em msg}. The user is responsible to assemble
   all fragments in the order they were generated. The {\em preserve\/}
   flag specifies whether the contents of fragment {\em frag\/} have to
   be preserved. If {\em preserve\/} is 0, the contents of {\em frag\/}
   may not be used after this call. Otherwise, the contents of {\em
     frag\/} are unchanged.
*/


/*
   \section{Fragment Interface} 

   To allow higher-level modules to save and retransmit fragments, the system
   layer provides explicit functions to create and manipulate fragments. By
   copying the contents of a fragment, the fragment can be resent after the
   next fragment is sent or received. To reduce the overhead of copying, the
   user can specify whether the contents of the source need to be preserved
   or not. If they need not be preserved, the implementation may choose to
   swap the contents instead of copying them.
*/

#define PAN_FRAGMENT_FIRST 0x01
#define PAN_FRAGMENT_LAST  0x02

extern pan_fragment_p pan_fragment_create(void);
/*
   Create a new fragment.
*/

extern void pan_fragment_clear(pan_fragment_p frag);
/*
   Clear fragment {\em frag}.
*/

extern void pan_fragment_nsap(pan_fragment_p frag, pan_nsap_p nsap);
/*
   XXX: Associates fragment {\em frag\/} with network service access point
   {\em nsap\/}. Remove this, or change into forward.
*/

extern int pan_fragment_copy(pan_fragment_p frag, pan_fragment_p copy,
			     int preserve);
/*
   Create a copy of the contents of fragment {\em frag} in fragment {\em
   copy}.  The old contents of copy become invalid. The {\em
   preserve\/} flag specifies whether the contents of fragment {\em
   frag\/} have to be preserved. If {\em preserve\/} is 0, the contents
   of {\em frag\/} may not be used after this call. Otherwise, the
   contents of {\em frag\/} are unchanged. The function returns 1 if the
   contents of {\em frag\/} are preserved, 0 otherwise. Note that the
   contents of {\em frag\/} may be preserved even when the {\em
     preserve\/} flag is 0.
*/

extern void *pan_fragment_header(pan_fragment_p frag);
/*
   Returns a pointer to the header part of fragment {\em frag}. The size
   of the header is determined by the nsap associated with {\em
     frag\/} via {\em pan\_msg\_fragment\/} or {\em
     pan\_fragment\_nsap\/}.
*/

extern int pan_fragment_flags(pan_fragment_p frag);
/*
   Returns the flags associated with {\em frag\/}. The flags indicate
   whether the given fragment is the first or last of the corresponding
   message.
*/

extern int pan_fragment_length(pan_fragment_p frag);
/*
   Returns the length of the data in fragment in bytes.
*/
...
/*
   ...
   possible to forward a received fragment via a different nsap, so the
...
extern void pan_nsap_fragment(pan_nsap_p nsap,
			      void rec_frag(pan_fragment_p frag), 
			      int header_size, int flags);
/*
   Initialize {\em nsap} to a network service access point for fragments
   with header size {\em header\_size}. Fragments received on this nsap
   are handled with an upcall to {\em rec\_frag}. {\em rec\_frag} gets as
   argument a pointer to the received fragment. The received fragment is
   only valid during the lifetime of the upcall, and it may not be
   cleared in the upcall. {\em flags\/} specifies the type of
...
/*
   ...
   sends fragments to the nsap associated with the fragment. The second
   type sends small data messages to the nsap specified in the call.
*/


extern void pan_comm_unicast_fragment(int dest, pan_fragment_p frag);
/*
   Send fragment {\em frag\/} to destination {\em dest\/}. The fragment
   is sent to the network service access point associated with fragment.
*/

extern void pan_comm_multicast_fragment(pan_pset_p pset, 
					pan_fragment_p frag);
/*
   Send fragment {\em frag\/} to the destinations specified in platform
   set {\em pset\/}. The fragment is sent to the network service access
   point associated with fragment.
*/
