...
void
pan_grp_handle_join(grp_hdr_p hdr, pan_fragment_p frag)
{
    pan_group_p     grp;
...
pan_group_join(char *name, void (*receive)(pan_msg_p))
{
    ...
    join_hdr = pan_fragment_header(frag);
    ...
    /*
     * Do _not_ call pan_message_clear() after a multicast!!
     * The fragment still has to be delivered at home.
     */
    return grp;
}
...
void
pan_grp_handle_leave(grp_hdr_p hdr, pan_fragment_p frag)
{
    pan_group_p     grp;
...
pan_group_leave(pan_group_p grp)
{
    ...
    msg  = pan_msg_create();
    frag = pan_msg_fragment(msg, pan_grp_nsap);
    leave_hdr = pan_fragment_header(frag);
