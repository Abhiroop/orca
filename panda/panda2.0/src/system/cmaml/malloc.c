/*
	...
	AUTHOR
		...
		to make it faster
	VERSION
		$Header: /usr/proj/panda/Repositories/gnucvs/panda2.0/src/system/cmaml/malloc.c,v 1.2 1996/03/08 10:58:50 tim Exp $
	DESCRIPTION
...
private check_mallinks(), calc_checksum(), check_work_empty();
private started_working_on(), stopped_working_on();
...
private void link_free_chunk(mallink *ml);
private void unlink_free_chunk(mallink *ml);
private mallink *first_present(int class);
private mallink *search_free_list(int class, unsigned int n);
...
private void truncate(mallink *ml, unsigned int size);
private void combine_chunks(mallink *ml1, mallink *ml2);
private mallink *create_chunk(char *p, unsigned int n);
...
char *
realloc(char *addr, unsigned int n)
...
private mallink *
search_free_list(int class, unsigned int n)
{
...
private mallink *
create_chunk(char *p, unsigned int n)
{
...
private void
truncate(mallink *ml, unsigned int size)
{
...
private void
combine_chunks(mallink *ml1, mallink *ml2)
{
...
static int dummy;               /* ANSI C does not like empty files. RAFB */
...
private mallink *free_list_entry();
...
private void
dump_mallink(char *s, mallink *ml)	{
	acquire_malout();
	if (s)
		fprintf(malout, "%s: ", s);
	fprintf(malout, "@: %ld;", (long)ml);
...
private void
check_mallinks(char *s)	{
	mallink *ml;
	size_type size;
	int i;
	char stat;
	
printf("check_mallinks(%s)\n", s);
	check_ml_last(s);
	stat = BEZET;
	for_all_mallinks(ml)	{
