...
empty(pan_sys_buffer_p buf)
{
	...
	pan_free(buf->b_data);
	buf->b_data = pan_malloc(PACKET_SIZE);
...
create(void) 
{
    ...
    buf = pan_malloc(sizeof(pan_sys_buffer_t));
...
{
    return (pan_sys_buffer_p)pan_sys_pool_get(&pool);
}
...
pan_sys_buffer_push(pan_sys_buffer_p buf, int size, int clear, int align)
{
	...
	if (buf->b_index == 0){
	    buf->b_size  = buf_size(size + 128, clear);
	    pan_free(buf->b_data);
	    buf->b_data  = pan_malloc(buf->b_size);
	    buf->b_index = size;
...
pan_sys_buffer_first(pan_sys_buffer_p buf, pan_fragment_p frag, int preserve)
{
	...
	if (frag->header->size > PACKET_SIZE){
	    /* Use free and malloc iso realloc to avoid copying */
	    pan_free(buf->b_data);
	    buf->b_data = pan_malloc(frag->header->size);
	...
	buf->b_size = frag->f_bufsize;

	/*
	 * Buffers at the receiving side need not have the
	 * same size as the corresponding buffer at the
	 * sending side. So forget about this assert for now.
	 */
#if 0
	assert(frag->header->size <= frag->f_bufsize);
#endif
