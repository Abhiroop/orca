...
pan_msg_push(pan_msg_p msg, int length, int align)
{
    ...
    p = pan_sys_buffer_push(CURRENT_BUFFER(msg), length, 
			    MAX_TOTAL_HDR_SIZE, align);
    if (p == NULL){
	if (msg->pm_used > msg->pm_index) {
	    msg->pm_index++;
	} else {
...
pan_msg_fragment(pan_msg_p msg, pan_nsap_p nsap)
{
    ...
    msg->pm_hdr_size = TOTAL_HDR_SIZE(frag);

    buf_data = pan_sys_buffer_data(msg->pm_buffer[0]);
    buf_size = pan_sys_buffer_size(msg->pm_buffer[0]);

    frag->data   = buf_data;
    frag->size   = MIN(buf_size, PACKET_SIZE - do_align(msg->pm_hdr_size,
						        UNIVERSAL_ALIGNMENT));
    frag->owner  = 0;
