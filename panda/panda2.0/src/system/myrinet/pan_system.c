...
pan_init(int *argc, char *argv[])
{
    ...
    j = 1;
    for(i = 3; i < *argc; i++){
	if (strcmp(argv[i], "-pan_panic_core") == 0) {
	    pan_sys_dump_core();
	} else if (strcmp(argv[i], "-pan_comm_verbose") == 0) {
	    pan_comm_verbose = 1;
	} else if (strncmp(argv[i], "-pan_thread_", 12) == 0) {
	    pan_sys_thread_option( argv[i]+12);
	} else{
	    argv[j++] = argv[i];
	}
    }
...
void pan_sys_va_get_params(void *dummy, ...)
{
	...
	if (strcmp("PAN_SYS_statistics", cmd) == 0) {
	    ...
	    b_size = pan_sys_sprint_comm_stat_size();
	    *comm_stats = pan_malloc(b_size);
	    pan_sys_sprint_comm_stats(*comm_stats);
	} else if (strcmp("PAN_SYS_mcast_sequencer", cmd) == 0) {
	    i = (int *)va_arg(args, int*);
	    pan_mcast_va_get_params(dummy, cmd, i, NULL);
	} else if (strcmp("PAN_SYS_mcast_statistics", cmd) == 0) {
	    comm_stats = va_arg(args, char **);
	    pan_mcast_va_get_params(dummy, cmd, comm_stats, NULL);
	} else {
	    printf("%2d: No such system parameter: \"%s\" -- ignored\n",
		    pan_my_pid(), cmd);
	    va_arg(args, void*);
