...
{
    pan_sys_pool_init(&frag_pool, POLICY_NORMAL, FRAGMENT_POOL_INIT,
		      create, clear, "Fragment frag_pool");
}

void 
pan_sys_fragment_end(void)
{
    pan_sys_pool_clear(&frag_pool);
}
...
pan_fragment_create(void)
{
    ...
    frag = (pan_fragment_p)pan_sys_pool_get(&frag_pool);
    return frag;
}
...
assert(((pool_entry_p)frag)->pool_mode == OUT_POOL_ENTRY);
    pan_sys_pool_put(&frag_pool, (pool_entry_p)frag);
}
...
pan_sys_fragment_nsap_pop(pan_fragment_p frag, int len)
{
	...
	len -=	nsap->hdr_size +		/* Skip user common header */
		FRAG_HDR_SIZE;			/* Get fragment header */
...
pan_sys_fragment_nsap_push(pan_fragment_p frag)
{
    ...
    assert(aligned(nsap_offset, UNIVERSAL_ALIGNMENT));
    assert(nsap_offset + NSAP_HDR_SIZE <= PACKET_SIZE);

    *(nsap_p)(frag->data + nsap_offset) = pan_sys_nsap_index(nsap);
...
pan_sys_fragment_nsap_look(pan_fragment_p frag)
{
    ...
    assert(nsap_offset + NSAP_HDR_SIZE <= PACKET_SIZE);
    nsap_id = *(nsap_p)(frag->data + nsap_offset);
